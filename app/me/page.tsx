      ...
      <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
        <div className="space-y-6">
          {!currentContext ? (
            <section className="rounded-[28px] border border-dashed border-[var(--border-default)] bg-[rgba(255,255,255,0.6)] p-5">
              ...
              <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">
                连接 SecondMe 后，系统会读取你的用户资料、兴趣标签和软记忆，并缓存 24 小时，供冒险生成稳定复用。
              </p>
              <Link
                href="/api/auth/login"
          ...
          {auth === "connected" ? (
            <section className="rounded-[28px] border border-[rgba(95,127,98,0.22)] bg-[rgba(231,239,230,0.86)] p-5">
              <p className="text-sm font-semibold text-[var(--accent-moss)]">已连接 SecondMe，缓存画像与动物人格都已经准备好，可以继续进入冒险。</p>
            </section>
          ) : null}

          {auth === "refreshed" ? (
            <section className="rounded-[28px] border border-[rgba(95,127,98,0.22)] bg-[rgba(231,239,230,0.86)] p-5">
              <p className="text-sm font-semibold text-[var(--accent-moss)]">SecondMe 会话已恢复，可以继续使用你的缓存画像进入冒险。</p>
            </section>
          ) : null}
          ...
          {auth === "required" ? (
            <section className="rounded-[28px] border border-[rgba(200,128,92,0.22)] bg-[rgba(255,245,238,0.86)] p-5">
              <p className="text-sm font-semibold text-[var(--apricot)]">请先连接 SecondMe，再进入冒险。</p>
            </section>
          ) : null}
          ...
          {setup === "story_schema_required" || !schemaStatus.ready ? (
            <section className="rounded-[28px] border border-[rgba(200,128,92,0.22)] bg-[rgba(255,245,238,0.86)] p-5">
              <p className="text-sm font-semibold text-[var(--apricot)]">
                冒险体验的数据表还没迁移完成，请先执行 `db/008_adventure_memory_refactor.sql`，然后刷新页面。
              </p>
            </section>
          ) : null}
          ...
          <section className="rounded-[32px] border border-[var(--border-light)] bg-[rgba(252,251,250,0.84)] p-6 shadow-[var(--shadow-medium)]">
            ...
            {timelineItems.length > 0 ? (
              ...
              <div className="mt-5 rounded-[22px] border border-dashed border-[var(--border-default)] bg-[rgba(255,255,255,0.56)] p-5">
                <p className="text-sm leading-7 text-[var(--text-secondary)]">你的档案馆还没有记录，先从首页进入一本书打开副本，让第一篇冒险先落下来。</p>
              </div>
            )}
          </section>
        </div>
        ...
        <div className="space-y-6">
          <StateCard
            ...
            title={currentContext ? "这里会继续长出你的冒险记录" : "连接后这里会变成你的专属故事档案馆"}
            description={
              currentContext
                ? "现在这页会把你创建的公开副本、你加入的别人的冒险和当前分身状态放在同一个档案馆里。"
                : "登录成功后，这里会出现你的动物人格、故事时间线和冒险参与。"
            }
          />
          ...
          <section className="rounded-[32px] border border-[var(--border-light)] bg-[rgba(252,251,250,0.84)] p-6 shadow-[var(--shadow-medium)]">
            ...
            <p className="mt-3 text-base leading-8 text-[var(--text-secondary)]">
              当前分享页仍然沿用动物人格主视图。等冒险体验稳定下来以后，这里还可以继续扩展副本卡片和多人冒险摘要。
            </p>
            <div className="mt-5">
              <Link
                href="/me/share"
